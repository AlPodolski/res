...
class Obmenka implements PaymentInterface
{
    ...
    public function getPayUrl($orderId, $sum, $city, $currency, $des = 'Пополнение кабинета rex')
    {
        ...
        if (isset($rayRequestResult->tracking) and $createPayUrl = $this->createPayUrl(
            ...
            if ($result) return $result->pay_link;
    ...
    private function createPay($orderId, $sum, $city, $currency, $des)
    {
        ...
        $data = [
            'payment_id' => $orderId.'-rex',
            "amount" => $sum,
            'currency' => $currency,
            "description" => $des,
            "success_url" => "https://".$city.".rexxx-sex.com/pay/".$orderId,
            "fail_url" => "https://".$city.".rexxx-sex.com/pay",
            "status_url" => "https://".$city.".rexxx-sex.com/pay/".$orderId ,
        ];
