    ...
    <a href="/post/{{$post->url}}" class="d-block image-wrap">
        @if(isset($post->photo))
            <picture>
                ...
                <source srcset="/314-441/thumbs{{str_replace('.jpg', '.webp', $post->photo)}}"
                ...
                <source srcset="/370-526/thumbs{{$post->photo}}" media="(max-width: 400px)" type="image/jpeg">

                <source srcset="/211-300/thumbs{{str_replace('.jpg', '.webp', $post->photo)}}" type="image/webp">
                <source srcset="/211-300/thumbs{{$post->photo }}" type="image/jpeg">
...
<g>
    ...
    @if($post->phone)
        <a
            ...
            data-id="{{ $post->id }}"
            onclick="show_phone(this)" class="phone">Показать телефон</a>
    @else
